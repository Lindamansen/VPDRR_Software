...
pred_dataset=pd.read_csv("./data/data_tui/data_tui.csv")
text_dataset=pd.read_csv("./data/data_tui/text_tui.csv")
sample_text_tui=pd.read_csv("./data/data_tui/sample_text_tui_1.csv")

drug_name=[]
disease_name=[]
score_all=[]
mode_all=[]
for i in pred_dataset.iloc[:,1]:
...
for j in pred_dataset.iloc[:,2]:
    score_all.append(round(j, 6))
text_number=[]
for i in text_dataset.iloc[:,0]:
...
for i in range(len(sample_text)):
    Rare_match=text_dataset.iloc[i][0]/sample_text.iloc[i][0]+text_dataset.iloc[i][0]/sample_text.iloc[i][1]
    rare_values.append(round(Rare_match, 3))

rare_value=['UnKnow' if i ==0 else i for i in rare_values]

sum_axis=[]
location=[]
for i in range(len(text_number[:19000])):
    sum=text_number[i]+score_all[i]
    axis=sigmoid(sum)
    sum_axis.append(round(axis,3))
    location.append(i)
sum_location=zip(sum_axis,location)
sum_locat_all=sorted(sum_location,key=lambda x: x[0],reverse=True)
result = zip(*sum_locat_all)
sigmoid_values, location= [list(x) for x in result]
Drug_name=[]
Disease_name=[]
Text_number=[]
Score_all=[]
